...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listCaseToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
                ...
                columns:[[
					...
					{field:'caseAcreage',align:'left',width:10,title:'面积'},
					...
					{field:'branchCompany',align:'left',width:35,title:'隶属分公司',
						formatter:function(value,row,index){
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","showCase?caseId="+row.caseId);
                },
	...
	$("#js_add").on("click",function(){
		  location.href = "caseSave.jsp";
	   /* if(!$("#js_layout").layout("panel","south").is(":visible")){
			$("#js_layout").layout("autoExpand","south");
	   }
	   $("#framebottom").attr("src","caseSave.jsp"); */
	});
    ...
    $("#js_edit").on("click",function(){
       ...
       if(roleIdArr.length==1){
    	   $("#framebottom").attr("src","showCase?caseId="+roleIdArr[0]);
       }else{
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'searchListCases'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
    /*window resize*/
    winResize=function(){
