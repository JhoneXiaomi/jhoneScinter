...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listFitMentToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","showfitMent?caseId="+row.caseId);
                },
	...
	$("#js_add").on("click",function(){
	   ...
	   $("#framebottom").attr("src","fitMentSave.jsp");
	});
    ...
    $("#js_edit").on("click",function(){
       ...
       if(roleIdArr.length==1){
    	   $("#framebottom").attr("src","showfitMent?caseId="+roleIdArr[0]);
       }else{
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'searchListFitMent'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
    /*window resize*/
    winResize=function(){
