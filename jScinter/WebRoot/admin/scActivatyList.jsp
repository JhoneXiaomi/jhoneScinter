		...
		<div id="toolbar" class="toolpanel datagrid-toolbar">
		    ...
		    <div class="extend-panel" id="extendPanel">
		        ...
		        <ul>
		            ...
		            <li>分类：<select name="sel" id="" class="easyui-combobox" style="width:100px;" data-options="panelHeight:'auto'"><option value="0">所有状态</option><option value="1">行业资讯</option><option value="2">装修活动</option></select></li>
		        </ul>
            ...
            <div class="rf reletive sch">
				<input class="txt" type="text" id="keyWord" name="keyWord" value="请输入活动标题、作者或关键字" />
...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listScActivatyToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
                ...
                columns:[[
					...
					{field:'type',align:'left',width:30,title:'活动分类'},
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showScActivaty?newsId="+row.newsId);
                },
	...
	$("#js_add").on("click",function(){
	   location.href = "scActivatySave.jsp";
	});
    ...
    $("#js_edit").on("click",function(){
       ...
       if(roleIdArr.length==1){
    	   $("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showScActivaty?newsId="+roleIdArr[0]);
       }else{
    ...
    $("#js_del").on("click",function(){
       ...
       if(roleIdArr.length>0){
    	   $.messager.confirm("确认", "确认删除选中的"+roleIdArr.length+"条活动?", function (r){  
	   	        if (r) {
	   	        	$.ajax({
	   		   			type : "POST",
	   		   			dataType : "json",
	   		   			url : "${pageContext.request.contextPath}/admin/delScActivaty",
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'${pageContext.request.contextPath}/admin/searchListAcActivaty'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
	...
	hideCenter=function(newsId){
		if($("#js_layout").layout("panel","center").is(":visible")){
			...
			$('#js_center').html("");
			$('#js_center').html('<iframe src="${pageContext.request.contextPath}/admin/showNews?newsId='+newsId+'" height="100%" width="100%" frameborder="0" scrolling="no" allowtransparency="true"></iframe>');
			//查看列表点击事件
