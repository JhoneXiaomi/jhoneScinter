	...
	<div data-options="region:'center'" class="noborder hidden">
		...
		<div id="toolbar" class="toolpanel datagrid-toolbar">
            ...
            <div class="rf reletive sch">
				<input class="txt" type="text" id="keyWord" name="keyWord" value="请输入轮播图片名称" />
...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listSyhdlbToJsonm',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
				...
				checkOnSelect:true,
           		selectOnCheck:false,
                columns:[[
					{field:'homeimgId',checkbox:true},
					{field:'homeimgName',align:'left',width:50,title:'轮播图名称'},
					{field:'homeimgAlt',align:'left',width:30,title:'轮播图ALT'},
					{field:'creater',align:'left',width:25,title:'创建者'},
					{field:'createTime',align:'left',width:25,title:'创建时间'},
					{field:'updater',align:'left',width:20,title:'更新者'},
					{field:'updateTime',align:'center',width:30,title:'更新时间'},
					{field:'branchCompany',align:'left',width:25,title:'隶属分公司',
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","showSyhdlbm?homeImgId="+row.homeimgId);
                },
	...
	$("#js_add").on("click",function(){
	   ...
	   $("#framebottom").attr("src","syhdlbmSave.jsp");
	});
    ...
    $("#js_edit").on("click",function(){
	   ...
	   //获取复选框值
	   var items = $("input[name='homeimgId']:checked");
       var homeImgIdArr = [];
       $.each(items, function (index, item) {
    	   homeImgIdArr[index] = item.value;
       });
       if(homeImgIdArr.length==1){
    	   $("#framebottom").attr("src","showSyhdlbm?homeImgId="+homeImgIdArr[0]);
       }else{
    ...
    $("#js_del").on("click",function(){
       ...
       if(homeImgIdArr.length>0){
    	   $.messager.confirm("确认", "确认删除选中的"+homeImgIdArr.length+"条案例?", function (r){  
	   	        if (r) {
	   	        	$.ajax({
	   		   			type : "POST",
	   		   			dataType : "json",
	   		   			url : "delSyhdlb",
	   		   			data : "homeImgIds=" + homeImgIdArr,
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'searchListSyhdlbm'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
    /*window resize*/
    winResize=function(){
