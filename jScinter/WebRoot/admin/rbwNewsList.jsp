...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listRbwNewsToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showRbwNews?newsId="+row.newsId);
                },
    ...
    $("#js_edit").on("click",function(){
       ...
       if(roleIdArr.length==1){
    	   $("#framebottom").attr("src","${pageContext.request.contextPath}/admin/showRbwNews?newsId="+roleIdArr[0]);
       }else{
    	   ...
    	   $.messager.confirm("确认", "确认删除选中的"+roleIdArr.length+"条新闻?", function (r){  
	   	        if (r) {
	   	        	$.ajax({
	   		   			type : "POST",
	   		   			dataType : "json",
	   		   			url : "${pageContext.request.contextPath}/admin/delRbwNews",
	...
	$("#search").on("click",function(){
		...
		$("#grid_recheck").datagrid({url:'${pageContext.request.contextPath}/admin/searchRbwListNews'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
	...
	hideCenter=function(newsId){
		if($("#js_layout").layout("panel","center").is(":visible")){
			...
			$('#js_center').html("");
			$('#js_center').html('<iframe src="${pageContext.request.contextPath}/admin/showRbwNews?newsId='+newsId+'" height="100%" width="100%" frameborder="0" scrolling="no" allowtransparency="true"></iframe>');
			//查看列表点击事件
