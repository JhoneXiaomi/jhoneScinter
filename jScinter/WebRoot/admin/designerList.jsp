	...
	<div data-options="region:'center'" class="noborder hidden">
		...
		<div id="toolbar" class="toolpanel datagrid-toolbar">
            ...
            <div class="rf reletive sch">
				<input class="txt" type="text" id="keyWord" name="keyWord" value="请输入设计师姓名、籍贯或毕业院校" />
...
jQuery(function($){
	  ...
	  var $$=window.$$={
        ...
        loadGrid:function(param){
            $("#grid_recheck").datagrid({
                url:'${pageContext.request.contextPath}/admin/listDesignerToJson',
                fit:true,
                striped: true,
                pagination:true,
                pageSize: 10,
				...
				checkOnSelect:true,
           		selectOnCheck:false,
                columns:[[
					{field:'designerId',checkbox:true},
					{field:'designerName',align:'left',width:20,title:'姓名'},
					{field:'designerSex',align:'left',width:10,title:'性别'},
					{field:'designerBirthPlace',align:'left',width:30,title:'籍贯',
						formatter:function(value,row,index){
                			value = value.replace("null","");
                			return value.replace(new RegExp(",","gm"),"");
						}	
					},
					{field:'designerTitle',align:'left',width:30,title:'职称'},
					{field:'designerWorkTime',align:'left',width:30,title:'从业时间'},
					{field:'designerPopularity',align:'left',width:15,title:'浏览量'},
					{field:'designerStyle',align:'left',width:40,title:'设计风格'},
					{field:'designerSchool',align:'left',width:40,title:'毕业院校'},
					{field:'designerDept',align:'left',width:30,title:'部门'},
					{field:'designerPrice',align:'left',width:20,title:'设计费用',
						formatter:function(value,row,index){
							value = value + "/㎡";
                			return value;
						}	
					...
					{field:'branchCompany',align:'left',width:30,title:'隶属分公司',
						formatter:function(value,row,index){
                ...
                onSelect:function(index,row){
					...
					$("#framebottom").attr("src","showDesigner?designerId="+row.designerId);
                },
	...
	$("#js_add").on("click",function(){
	   ...
	   $("#framebottom").attr("src","designerSave.jsp");
	});
    ...
    $("#js_edit").on("click",function(){
	   ...
	   //获取复选框值
	   var items = $("input[name='designerId']:checked");
       var designerIdArr = [];
       $.each(items, function (index, item) {
    	   designerIdArr[index] = item.value;
       });
       if(designerIdArr.length==1){
    	   $("#framebottom").attr("src","showDesigner?designerId="+designerIdArr[0]);
       }else{
    ...
    $("#js_del").on("click",function(){
       ...
       if(designerIdArr.length>0){
    	   $.messager.confirm("确认", "确认删除选中的"+designerIdArr.length+"条设计师?", function (r){  
	   	        if (r) {
	   	        	$.ajax({
	   		   			type : "POST",
	   		   			dataType : "json",
	   		   			url : "delDesigner",
	   		   			data : "designerIds=" + designerIdArr,
	...
	$("#search").on("click",function(){
		if($('#keyWord').val()=="请输入设计师姓名、籍贯或毕业院校"){
			$('#keyWord').val('');
		}
		...
		$("#grid_recheck").datagrid({url:'searchListDesigner'}); //重新加载查询url
		$("#grid_recheck").datagrid('reload'); //重新加载
	});
