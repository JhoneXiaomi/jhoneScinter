        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    imgUrl = imgUrl.substring(imgUrl.indexOf("/uedtior_upload/image/"),imgUrl.length);
                    //console.log(''+imgUrl);
                    img.setAttribute('src', urlPrefix + imgUrl + (imgUrl.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlPrefix + imgUrl);
